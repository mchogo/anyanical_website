                  ...
                  {detail.records.map((r) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span
                          className={`w-24 shrink-0 font-mono font-semibold ${r.pnl >= 0 ? 'text-emerald-300' : 'text-rose-300'}`}
                        >
                          {r.pnl >= 0 ? '+' : ''}
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-white/10 text-left text-xs text-slate-500">
                    <th className="sticky left-0 z-10 bg-slate-900 pb-2 pr-4 font-semibold">
                      銘柄
                    </th>
                ...
                <tbody className="divide-y divide-white/[0.04]">
                  {symbols.map((sym) => {
                    ...
                    return (
                      <tr key={sym}>
                        <td className="sticky left-0 z-10 bg-slate-900 py-2 pr-4 font-semibold text-white">
                          {sym}
                        </td>
                        ...
                        <td className="py-2">
                          <div className="flex flex-wrap gap-1">
                            {users.map((u) => (
                              <span
                                key={u.discordUserId + u.symbol}
                                className={`text-xs ${DIRECTION_COLORS[u.direction]}`}
                                title={`信頼度${u.confidence}% ${u.note}`}
                              >
                                {u.discordUserId.slice(-6)}{' '}
                                {DIRECTION_LABELS[u.direction]}
                              </span>
                            ))}
                          </div>
                        </td>
        ...
        <div className="space-y-2">
          {sorted.map(([code, count]) => (
            <div key={code} className="flex items-center gap-3">
              <span className="w-14 shrink-0 font-mono text-sm font-bold text-amber-200">
                {code}
              </span>
        ...
        <div className="max-h-72 overflow-y-auto space-y-1">
          {data.slice(0, 50).map((r, i) => (
            <div key={i} className="flex items-center gap-3 text-xs">
              ...
              <span className="w-14 shrink-0 font-mono font-bold text-amber-200">
                {r.typeCode}
              </span>
              <span className="font-mono text-slate-600">
                {r.discordUserId.slice(-8)}
              </span>
            </div>
          ))}
        </div>
...
export const AdminPage = ({ auth }: { auth: DiscordAuth }) => {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      if (!ov || !us) {
        setError(
          '管理者権限が設定されていません。workerのADMIN_USER_IDSを確認してください。',
        );
        return;
      }
      setOverview(ov);
            ...
            {users.length === 0 ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-white/10 text-left text-xs text-slate-500">
                      <th className="sticky left-0 z-10 bg-slate-900 pb-3 pr-4 font-semibold">
                        Discord ID
                      </th>
                  ...
                  <tbody className="divide-y divide-white/[0.04]">
                    {users.map((u) => (
                      ...
                      >
                        <td className="sticky left-0 z-10 bg-slate-900 py-3 pr-4 font-mono text-slate-300">
                          {u.discordUserId}
                        </td>
